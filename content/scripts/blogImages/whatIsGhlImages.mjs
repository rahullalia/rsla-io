 ...
 * whatIsGhlImages.mjs
 *
 * Generates 3 new inline images for the what-is-go-high-level post rewrite.
 * Removes old images, generates via Gemini, uploads to Sanity, inserts into body.
 *
 * Usage: node scripts/whatIsGhlImages.mjs [--dry-run]
 */
...
const DOC_ID = 'yfy9cLha6032ya4d21mg28';
...
const IMAGES = [
  {
    ...
    subject: 'tool consolidation overview',
    details: 'a central rounded rectangle labeled with a simple house icon, with 8 smaller circles around it connected by arrows pointing inward, each circle containing a simple icon (envelope, calendar, phone, chat bubble, funnel, star, gear, dollar sign), blue and red markers on whiteboard',
    alt: 'Whiteboard sketch showing eight business tools consolidating into one GoHighLevel platform',
    caption: 'One platform instead of eight separate subscriptions. That is the pitch.',
    afterH2Index: 1, // After "What you're actually getting inside GHL"
  },
  {
    style: 'notebookPencil',
    subject: 'automation workflow sketch',
    details: 'a simple linear flow drawn in pencil: a box labeled "booking" with an arrow to a box labeled "text sent" with an arrow to a box labeled "confirmed", with a small checkmark next to each box, on lined notebook paper, simple and clean',
    alt: 'Notebook sketch of a simple GoHighLevel automation: booking triggers text confirmation',
    caption: 'The first automation I built. Calendar booking sends a text confirmation. Still running unchanged.',
    afterH2Index: 2, // After "The first thing I set up"
  },
  {
    style: 'dashboardSketch',
    subject: 'AI agent studio wireframe',
    details: 'a rough wireframe of an agent builder interface, showing connected nodes in a flow: a start node, a knowledge base node, a router node splitting into two paths, and an end node, with small labels hand-written next to each, pencil on white paper with blue pen accents',
    alt: 'Hand-drawn wireframe of GoHighLevel Agent Studio showing connected AI workflow nodes',
    caption: 'Agent Studio lets you build AI agents with knowledge bases, routers, and API connections. All inside GHL.',
    afterH2Index: 3, // After "Agent Studio and the AI features"
  },
];
...
async function run() {
  console.log(`\nWhat Is GHL: Generate 3 Inline Images (${DRY_RUN ? 'DRY RUN' : 'LIVE'})\n`);
  ...
  for (let i = 0; i < IMAGES.length; i++) {
    ...
    const fn = `what-is-ghl-inline-${i + 1}.png`;
