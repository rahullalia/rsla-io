...
export default function StartHere() {
    ...
    return (
        <main ref={pageRef} className="min-h-screen">
            ...
            {/* Learn Section */}
            <section className="start-learn bg-sand py-20 md:py-28 px-6 md:px-12">
                <div className="max-w-4xl mx-auto">
                    <span className="font-mono text-accent text-xs uppercase tracking-widest font-bold">Learn</span>
                    <h2 className="font-sans font-bold text-2xl md:text-4xl text-dark tracking-tight mt-4 mb-12">
                        Pick your depth
                    </h2>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {learnTiers.map((tier) => (
                            <div key={tier.time} className="start-card bg-white border border-dark/5 rounded-2xl p-8 flex flex-col shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <h3 className="font-sans font-bold text-lg text-dark mb-3">{tier.label}</h3>
                                <p className="font-body text-dark/60 text-sm leading-relaxed mb-6 flex-grow">{tier.desc}</p>
                                {tier.external ? (
                                    <a href={tier.link} target="_blank" rel="noopener noreferrer" className="link-underline inline-flex items-center gap-2 font-sans font-bold text-accent text-sm hover:text-dark transition-colors group">
                                        {tier.linkText}
                                    ...
                                    <a href={tier.link} className="link-underline inline-flex items-center gap-2 font-sans font-bold text-accent text-sm hover:text-dark transition-colors group">
                                        {tier.linkText}
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="group-hover:translate-x-1 transition-transform"><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>
                                    </a>
                                ) : (
                                    <Link to={tier.link} className="link-underline inline-flex items-center gap-2 font-sans font-bold text-accent text-sm hover:text-dark transition-colors group">
                                        {tier.linkText}
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="group-hover:translate-x-1 transition-transform"><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>
                                    </Link>
                                )}
            ...
            {/* Follow Section */}
            <section className="start-follow bg-dark py-20 md:py-28 px-6 md:px-12">
                <div className="max-w-4xl mx-auto">
                    <div className="start-section">
                        <span className="font-mono text-accent text-xs uppercase tracking-widest font-bold">Follow</span>
                        <h2 className="font-sans font-bold text-2xl md:text-4xl text-white tracking-tight mt-4 mb-4">
                            Stay in the loop
                        </h2>
                    ...
                    <div className="start-section grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {socials.map((social) => (
                            <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="group bg-white/5 border border-white/10 rounded-xl p-5 hover:border-accent/30 transition-colors">
                                <h3 className="font-sans font-bold text-white mb-1 group-hover:text-accent transition-colors">{social.name}</h3>
                                <p className="font-body text-white/40 text-sm">{social.desc}</p>
                            </a>
                        ))}
                        <Link to="/blog" className="group bg-white/5 border border-white/10 rounded-xl p-5 hover:border-accent/30 transition-colors">
                            <h3 className="font-sans font-bold text-white mb-1 group-hover:text-accent transition-colors">Blog</h3>
                            <p className="font-body text-white/40 text-sm">Long form, searchable, evergreen</p>
                        </Link>
                    </div>
                </div>
            </section>
            ...
            <section className="bg-sand py-20 md:py-28 px-6 md:px-12">
                <div className="max-w-3xl mx-auto text-center">
                    <h2 className="font-sans font-bold text-2xl md:text-4xl text-dark tracking-tight mb-4">
                    ...
                    <Link to="/how-it-works" className="inline-flex items-center gap-2 font-sans font-bold text-accent text-sm hover:text-dark transition-colors group">
                        See how it works
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="group-hover:translate-x-1 transition-transform"><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>
                    </Link>
                </div>
